      ...
      <Card className="w-full max-w-lg rounded-3xl p-7 text-center shadow-lg">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {failed
            ...
            : "Estamos aguardando a confirmação oficial da Stripe. O pedido só entra no sistema depois que o webhook confirmar o pagamento."}
        </p>
        ...
        {!failed && (
          <div className="mt-5 flex items-start gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-left text-xs leading-relaxed text-emerald-900">
            ...
            <span><strong>Pagamento seguro.</strong> Não feche esta tela até a confirmação. Normalmente isso acontece em poucos segundos após a Stripe reconhecer o pagamento.</span>
          </div>
        )}
        {checkout?.status === "payment_pending" && (
          <div className="mt-4 flex items-center justify-center gap-2 text-xs font-semibold text-muted-foreground"><Clock className="size-4" /> Aguardando confirmação da Stripe</div>
        )}
